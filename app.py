...
app.config["SQLALCHEMY_DATABASE_URI"] = os.environ.get('DATABASE_URL') or "postgresql://postgres:Anhchuc123@localhost/HW5_DB"
## Provided:
...
class UpdatePriorites(FlaskForm):
    items = FloatField("Update the priority of this item", validators=[Required()])
    submit = SubmitField("Update")
...
def update(item):
    form = UpdatePriorites()
    if form.validate_on_submit():
        up = TodoItem.query.filter_by(description=item).first()
        up.priority = form.items.data
        db.session.commit()
...
def delete(lst):
    form = DeleteButtonForm()
    delete = TodoList.query.filter_by(title=lst).first()
